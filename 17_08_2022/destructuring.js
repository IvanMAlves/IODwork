...
//Exercise 1: Count and sort the family name of students => [{"Smith": 10},  {"Nguyen": 3}]

//Exercise 2: Get the student having the highest score (exam, quiz and homework)

//Exercise 3: Get the total score of each type based on family name (exam, quiz, homework) => Example


let SameLastNameCount = 0;
let SameLastName = "";

for (i = 0; i < students.length; i++){
    if(students[i].name.includes("Smith")){
        SameLastName = students[i].name.split(" ");
        SameLastNameCount += 1;
    }
}
...
let snameArr = []
  students.forEach((item) => {
        [fname, surname] = item.name.split(" ");
        snameArr.push(surname);
  })

  var groupedSname = snameArr.reduce(function(prev, cur) {
    prev[cur] = (prev[cur] || 0) + 1;
    return prev;
  }, {});

  console.log(groupedSname);
*/
...
//Exercise 4: Create a function to sort the family name based on their score type ("exam", "quiz", "homework", "total_score")
