...
let movie = movies["Marvel Cinematic Universe"];
const movieDetails = Object.values(movie);
{
let budgetsTotal = 0;
for (i = 0; i < movieDetails.length; i++){
    if(movieDetails[i].budget != "tbd"){
        let movieBudget = parseFloat(movieDetails[i].budget.replace(/,/g, ''));
        budgetsTotal += movieBudget;
    }
}

console.log("The total Budget is: " + budgetsTotal);
...
for (let i = 0; i <movieDetails.length; i++){
    if(!moviesByYear.has(movieDetails[i].release_year)){
        moviesByYear.set(movieDetails[i].release_year, [movieDetails[i].title]); //year is the key and title is the value in a map function
    } else {
        moviesByYear.get(movieDetails[i].release_year).push(movieDetails[i].title);
    }
}
...
let actionMovies = [];
for (let i = 0; i <movieDetails.length; i++){
    if(movieDetails[i].category_name == 'Action'){
        actionMovies.push(movieDetails[i].title);
    }
}
console.log('Movies with Action as the category: ');
