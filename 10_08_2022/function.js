...
function highestSal() {
  ...
  if (list.length > 0){ //this makes sure that there is atleast 1 user left in the list
    ...
    let index = 0; //index is variable used to be to determine the first name and last name and salary in the array
    for(let i = 1; i < list.length; i++) { //for loop to go through the array
      if(list[i].salary > highestSalary) { //checks the current item in the array against the currently stored highest salary
        ...
        index = i; //setting index to i so the loop knows when to break
      }
    }

    display.innerText = list[index].firstName + " " + list[index].lastName + " with a salary of "+ list[index].salary + " salary.";
    list.splice(index, 1); //actually gets rid of the selected item in the array
